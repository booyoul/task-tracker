...
5. **레이아웃 가변성 및 가로 스크롤 방지 규칙**: 트래커 이름이나 긴 설명글이 들어가는 드롭다운 및 팝업 요소는 고정 폭(예: `w-72`)을 지양하고 콘텐츠 양에 맞추어 유연하게 조절되도록 `w-max`와 최소/최대 너비 제약(`min-w-...`, `max-w-...`)을 조합하십시오. 가로 넘침 및 스크롤바 노출을 막기 위해 리스트 컨테이너에는 `overflow-x-hidden`을 명시해야 합니다.
