...
function getKanbanColumns(){
  return [
    ...
    {key:'CANCELLED',title:'취소',hint:'취소된 업무',color:'slate'}
  ];
}
...
function buildKanbanCard(t,today){
  ...
  return `<article class="rounded-2xl border ${accent} bg-white p-3 shadow-sm">
    ...
    <div class="mt-3 grid grid-cols-4 gap-1">
      ...
      <button type="button" class="mobile-status-btn rounded-xl bg-slate-100 px-2 py-1.5 text-[11px] font-black text-slate-600" data-id="${escapeHTML(t.id)}" data-status="CANCELLED">취소</button>
    </div>
  </article>`;
}
function renderKanbanView(filtered){
  ...
  const activeTotal=list.filter(t=>getEffectiveStatus(t,today)!=='CANCELLED').length;
  const pct=activeTotal?Math.round(completed/activeTotal*100):0;
  ...
  board.className='grid grid-cols-1 gap-3 lg:grid-cols-5';
