...
async function initializeAuthAndData() {
  if (isFirebaseAvailable && auth) {
    ...
    try {
      await initAuth();
      auth.onAuthStateChanged(user => { isAuthReady = !!user; if (user) fetchInitialData(); });
    } catch (e) { console.error('Auth initialization failed', e); updateUI(); }
  } else updateUI();
}
function restoreCurrentTrackerSelection() {
  try { const saved = localStorage.getItem('flow_current_tracker'); if (saved && trackers.some(t => t.id === saved)) currentTrackerId = saved; }
  catch (e) { console.warn('Tracker restore skipped because localStorage is unavailable.', e); }
  updateTrackerUI();
}
