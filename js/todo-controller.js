...
function getTodoCalendarItemClass(todo) {
  if (todo.completed === true) return 'border-slate-200 bg-slate-100 text-slate-500 line-through dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400';
  if (isTodoOverdue(todo)) return 'border-rose-200 bg-rose-100 text-rose-700 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-300';
  return 'border-violet-200 bg-violet-100 text-violet-700 dark:border-violet-900 dark:bg-violet-950/50 dark:text-violet-300';
}
...
function syncTodoCalendarControls() {
  ...
  const activeClass = 'rounded-lg bg-white px-3 py-1.5 text-xs font-bold text-slate-800 shadow-sm dark:bg-slate-700 dark:text-slate-100';
  const inactiveClass = 'rounded-lg px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100';
  ...
  for (let index = 0; index < fullCells; index += 1) {
    ...
    cells.push(`
      <div ${dateString ? `data-todo-calendar-date="${dateString}"` : ''}
        class="${dateString ? 'bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800' : 'bg-slate-50 dark:bg-slate-950'} border-b border-r border-slate-100 transition-colors dark:border-slate-700"
  ...
  content.innerHTML = `
    <div data-todo-calendar-view="desktop-month" class="overflow-x-auto rounded-xl pb-2">
      <div class="relative min-w-[900px] lg:min-w-0">
        <div class="grid grid-cols-7 gap-px overflow-hidden rounded-t-lg border border-slate-200 bg-slate-200 dark:border-slate-700 dark:bg-slate-700">
          ${TODO_CALENDAR_WEEKDAYS.map((weekday, index) => `<div class="bg-slate-50 py-2 text-center text-xs font-semibold dark:bg-slate-800 ${
            index === 0 ? 'text-rose-500' : index === 6 ? 'text-blue-500' : 'text-slate-500'
          }">${weekday}</div>`).join('')}
        </div>
        <div class="relative mt-px overflow-hidden rounded-b-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900" data-week-lane-counts="${weekLayouts.map(layout => layout.laneCount).join(',')}">
          <div class="relative z-0 grid grid-cols-7 gap-px bg-slate-200 dark:bg-slate-700">${cells.join('')}</div>
      ...
      <section data-todo-calendar-date="${dateString}">
        <div class="mb-2 mt-4 flex items-center gap-2 first:mt-0">
          <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${
            today ? 'bg-indigo-600 text-white shadow-sm' : `bg-slate-100 dark:bg-slate-800 ${dayClass}`
          }">${day}</span>
        ...
        <div class="space-y-2">
          ${dayItems.map(todo => {
            ...
            return `
              <button type="button" data-todo-calendar-id="${escapeHTML(todo.id)}"
                class="block w-full rounded-xl border bg-white p-3 text-left shadow-sm transition active:scale-[0.98] dark:bg-slate-800 ${getTodoCalendarItemClass(todo)}">
...
function renderTodoDesktopYearCalendar(content, yearItems, year) {
  ...
  const monthButtons = Array.from({ length: 12 }, (_, month) => `
    <button type="button" data-todo-calendar-month="${year}-${String(month + 1).padStart(2, '0')}"
      class="py-3 text-center text-xs font-bold text-slate-700 transition-colors hover:bg-slate-100 hover:text-indigo-600 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-indigo-300">${month + 1}월</button>
  `).join('');
  const tiles = Array.from({ length: 12 }, (_, month) => `
    <button type="button" data-todo-calendar-month-target="${year}-${String(month + 1).padStart(2, '0')}"
      class="border-b border-slate-100 bg-white transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800" style="height:${bodyHeight}px" aria-label="${month + 1}월 월간 보기"></button>
  `).join('');
  ...
  content.innerHTML = `
    <div data-todo-calendar-view="desktop-year" class="overflow-x-auto rounded-xl pb-2">
      <div class="min-w-[900px] lg:min-w-0">
        <div class="grid grid-cols-12 gap-px border-b border-slate-200 bg-slate-50 shadow-sm dark:border-slate-700 dark:bg-slate-800">${monthButtons}</div>
        <div class="relative overflow-hidden rounded-b-xl border border-t-0 border-slate-200 dark:border-slate-700">
          <div class="grid grid-cols-12 gap-px bg-slate-100/50 dark:bg-slate-700">${tiles}</div>
...
function renderTodoMobileYearCalendar(content, yearItems, year) {
  ...
  const monthRows = Array.from({ length: 12 }, (_, month) => `
    <button type="button" data-todo-calendar-month="${year}-${String(month + 1).padStart(2, '0')}"
      class="flex h-[54px] w-full items-center justify-center border-b border-slate-100 text-[11px] font-bold text-slate-500 hover:bg-slate-50 hover:text-indigo-600 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-indigo-300">${month + 1}월</button>
  `).join('');
  const gridRows = Array.from({ length: 12 }, () => '<div class="h-[54px] border-b border-slate-100 dark:border-slate-700"></div>').join('');
  ...
  content.innerHTML = `
    <div data-todo-calendar-view="mobile-year" class="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div class="grid grid-cols-[48px_1fr]" style="height:${rowHeight * 12}px">
        <div class="z-10 border-r border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">${monthRows}</div>
        <div class="relative min-w-0 overflow-hidden">
          <div class="absolute inset-0">${gridRows}</div>
          <div class="pointer-events-none absolute inset-0">${bars}</div>
        </div>
      </div>
      ${yearItems.length > displayedItems.length ? `<p class="border-t border-slate-100 px-3 py-2 text-[10px] font-semibold text-slate-500 dark:border-slate-700 dark:text-slate-400">화면 맞춤을 위해 시작일이 빠른 ${displayedItems.length}개를 표시합니다. 전체 ${yearItems.length}개</p>` : ''}
    </div>
  `;
}
...
function syncTodoViewMode() {
  ...
  const activeClass = 'rounded-lg bg-white px-4 py-1.5 text-xs font-semibold text-slate-800 shadow-sm transition dark:bg-slate-700 dark:text-slate-100';
  const inactiveClass = 'rounded-lg px-4 py-1.5 text-xs font-semibold text-slate-500 transition hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100';
  if (listButton) {
...
function getTodoDateLabel(todo, today = getTodayStr()) {
  if (isTodoOverdue(todo, today)) return { label: `기한 경과 · ${todo.dueDate}`, className: 'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900 dark:bg-rose-950/50 dark:text-rose-300' };
  if (todoOverlapsRange(todo, today, today)) return { label: '오늘 할 일', className: 'border-indigo-200 bg-indigo-50 text-indigo-700 dark:border-indigo-900 dark:bg-indigo-950/50 dark:text-indigo-300' };
  return { label: `${todo.startDate} ~ ${todo.dueDate}`, className: 'border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300' };
}
...
function resolveTodoTaskLink(taskLink) {
  ...
  const subTask = normalized.subTaskId && task
    ...
    : null;
  const occurrence = normalized.occurrenceKey && subTask
    ? getTodoLinkOccurrence(subTask, normalized.occurrenceKey)
    : null;
  if (!tracker || !task || (normalized.subTaskId && !subTask) || (normalized.occurrenceKey && !occurrence)) {
  ...
  if (occurrence) parts.push(`${formatTodoOccurrenceDate(occurrence)} 회차`);
  return {
    available: true,
    taskLink: normalized,
    tracker,
...
function formatTodoOccurrenceDate(occurrence = {}) {
  const start = String(occurrence.startDate || occurrence.occurrenceKey || '');
  const due = String(occurrence.dueDate || start);
  if (!start) return '날짜 미정';
  return start === due ? start.substring(5) : `${start.substring(5)}~${due.substring(5)}`;
}

function getTodoLinkOccurrence(subTask, occurrenceKey) {
  if (
    subTask?.recurrence?.enabled !== true ||
    !/^\d{4}-\d{2}-\d{2}$/.test(occurrenceKey) ||
    typeof getRecurringSubTaskOccurrences !== 'function'
  ) return null;
  return getRecurringSubTaskOccurrences(subTask, occurrenceKey, occurrenceKey, occurrenceKey)
    .find(item => item.occurrenceKey === occurrenceKey) || null;
}

function getTodoLinkOccurrences(subTask, anchorDate = '') {
  if (subTask?.recurrence?.enabled !== true || typeof getRecurringSubTaskOccurrences !== 'function') return [];
  const anchor = /^\d{4}-\d{2}-\d{2}$/.test(anchorDate) ? anchorDate : getTodayStr();
  const anchorYear = Number.parseInt(anchor.slice(0, 4), 10);
  const frequency = subTask.recurrence.frequency || 'WEEKLY';
  const interval = Math.max(1, Number.parseInt(subTask.recurrence.interval, 10) || 1);
  const occurrenceSpanDays = frequency === 'DAILY'
    ? interval * 12
    : frequency === 'WEEKLY'
      ? interval * 12 * 7
      : frequency === 'MONTHLY'
        ...
        : frequency === 'QUARTERLY'
          ? interval * 12 * 92
          : interval * 12 * 366;
  const spanYears = Math.max(1, Math.ceil(occurrenceSpanDays / 365));
  const rangeStart = `${anchorYear - spanYears}-01-01`;
  const rangeEnd = `${anchorYear + spanYears}-12-31`;
  const anchorTime = new Date(`${anchor}T00:00:00`).getTime();
  return getRecurringSubTaskOccurrences(subTask, rangeStart, rangeEnd, anchor)
    .sort((a, b) => {
      const distance = Math.abs(new Date(`${a.occurrenceKey}T00:00:00`).getTime() - anchorTime);
      const otherDistance = Math.abs(new Date(`${b.occurrenceKey}T00:00:00`).getTime() - anchorTime);
      return distance - otherDistance || String(a.occurrenceKey).localeCompare(String(b.occurrenceKey));
    })
    .slice(0, 12)
    .sort((a, b) => String(a.occurrenceKey).localeCompare(String(b.occurrenceKey)));
}

function populateTodoLinkOccurrences(subTask, selectedOccurrenceKey = '') {
  ...
  const isRecurring = subTask?.recurrence?.enabled === true;
  if (field) {
    field.hidden = !isRecurring;
    field.classList.toggle('hidden', !isRecurring);
  }
  if (!isRecurring) {
    replaceTodoSelectOptions(select, '반복 전체(회차 미지정)', [], item => item.title);
    return;
  }
  const anchor = selectedOccurrenceKey || document.getElementById('input-todo-start')?.value || getTodayStr();
  const occurrences = getTodoLinkOccurrences(subTask, anchor);
  replaceTodoSelectOptions(select, '반복 전체(회차 미지정)', occurrences.map(item => ({ ...item, id: item.occurrenceKey })), item => {
    const status = typeof getStatusKorean === 'function' ? getStatusKorean(item.status).replace('됨', '') : item.status;
    return `${formatTodoOccurrenceDate(item)} · ${status}`;
  });
  if (select) select.value = occurrences.some(item => item.occurrenceKey === selectedOccurrenceKey) ? selectedOccurrenceKey : '';
}

function populateTodoLinkSubTasks(taskId, selectedSubTaskId = '', selectedOccurrenceKey = '') {
  ...
  populateTodoLinkOccurrences(subTasks.find(item => item.id === subTaskSelect?.value), selectedOccurrenceKey);
}

function populateTodoLinkTasks(trackerId, selectedTaskId = '', selectedSubTaskId = '', selectedOccurrenceKey = '') {
  const taskSelect = document.getElementById('input-todo-link-task');
  ...
  populateTodoLinkSubTasks(taskSelect?.value || '', selectedSubTaskId, selectedOccurrenceKey);
}
...
function syncTodoTaskLinkInputs(taskLink = null) {
  ...
  if (resolved.available) {
    ...
    populateTodoLinkTasks(resolved.tracker.id, resolved.task.id, resolved.subTask?.id || '', resolved.occurrence?.occurrenceKey || '');
...
function getTodoTaskLinkFromInputs() {
  ...
  const occurrenceKey = document.getElementById('input-todo-link-occurrence')?.value || '';
  if (!trackerId) {
    return {
  ...
  const taskLink = window.normalizeTodoTaskLink?.({ trackerId, taskId, subTaskId, occurrenceKey });
...
function renderTodoView() {
  ...
  document.querySelectorAll('[data-todo-date-filter]').forEach(button => {
    ...
    button.className = active
      ...
      : 'rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-indigo-700 dark:hover:text-indigo-300';
  });
  ...
  container.innerHTML = '';
  if (!visible.length) {
    container.innerHTML = `<div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-12 text-center dark:border-slate-700 dark:bg-slate-800">
  ...
  visible.forEach(todo => {
    ...
    const linkedTaskHtml = linkedTask?.available
      ? `<button type="button" class="btn-open-todo-task-link inline-flex max-w-full items-center gap-1 rounded-lg border border-violet-200 bg-violet-50 px-2 py-1 text-left text-[11px] font-bold text-violet-700 transition hover:border-violet-300 hover:bg-violet-100 dark:border-violet-900 dark:bg-violet-950/50 dark:text-violet-300 dark:hover:border-violet-700 dark:hover:bg-violet-950/70" data-id="${escapeHTML(todo.id)}" title="${escapeHTML(linkedTask.label)}" aria-label="연결된 업무 열기: ${escapeHTML(linkedTask.label)}"><span aria-hidden="true">↗</span><span class="truncate">${escapeHTML(linkedTask.label)}</span></button>`
      : todo.taskLink
        ? '<span class="inline-flex rounded-lg border border-amber-200 bg-amber-50 px-2 py-1 text-[11px] font-bold text-amber-700 dark:border-amber-900 dark:bg-amber-950/50 dark:text-amber-300">연결된 업무를 볼 수 없음</span>'
        : '';
    const card = document.createElement('article');
    card.className = `rounded-2xl border p-4 shadow-sm transition ${todo.completed ? 'border-slate-100 bg-slate-50/70 opacity-75 dark:border-slate-700 dark:bg-slate-800/70' : isTodoOverdue(todo) ? 'border-rose-200 bg-rose-50/40 dark:border-rose-900 dark:bg-rose-950/30' : 'border-slate-100 bg-white hover:border-indigo-200 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-indigo-700'}`;
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0">
              <h3 class="break-words text-sm font-black text-slate-800 dark:text-slate-100 ${todo.completed ? 'line-through text-slate-400' : ''}">${escapeHTML(todo.title)}</h3>
              ${todo.memo ? `<p class="mt-1 break-words text-xs leading-relaxed text-slate-500 dark:text-slate-400">${escapeHTML(todo.memo)}</p>` : ''}
            </div>
            <div class="flex shrink-0 items-center gap-1.5 self-end sm:self-start">
              <button type="button" class="btn-edit-todo rounded-lg px-2 py-1 text-xs font-bold text-indigo-600 hover:bg-indigo-50 dark:text-indigo-300 dark:hover:bg-indigo-950/50" data-id="${escapeHTML(todo.id)}">수정</button>
              <button type="button" class="btn-delete-todo rounded-lg px-2 py-1 text-xs font-bold text-rose-500 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/50" data-id="${escapeHTML(todo.id)}">삭제</button>
            </div>
          </div>
...
function initTodoController() {
  ...
  document.getElementById('input-todo-start')?.addEventListener('change', () => {
    ...
    const taskId = document.getElementById('input-todo-link-task')?.value || '';
    const subTaskId = document.getElementById('input-todo-link-subtask')?.value || '';
    const selectedOccurrenceKey = document.getElementById('input-todo-link-occurrence')?.value || '';
    const task = getTodoLinkTasks(trackerId).find(item => item.id === taskId);
    populateTodoLinkOccurrences((task?.subTasks || []).find(item => item.id === subTaskId), selectedOccurrenceKey);
  });
  document.querySelectorAll('[data-todo-date-filter]').forEach(button => {
