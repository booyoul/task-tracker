console.info('Smart Task Flow schema-service.js v20260626-module-split-phase1 loaded');
// Task schema normalization, validation, and Firebase migration helpers.
...
function normalizeSubTaskForSchema(st = {}, parent = {}) {
  ...
  return {
    ...
    assignee: st.assignee || parent.assignee || '미지정',
    startDate: start > due ? due : start,
    dueDate: due,
...
function taskNeedsSchemaMigration(task = {}) {
  return task.schemaVersion !== CURRENT_TASK_SCHEMA_VERSION || !task.industry || !task.taskType || !Array.isArray(task.subTasks) || (task.subTasks || []).some(st => !st.id || !st.status || !st.startDate || !st.dueDate);
}
async function migrateExistingTasksToCurrentSchema(scopeTasks = tasks) {
  ...
  try {
    ...
    candidates.forEach(t => batch.set(coll.doc(t.id), normalizeTaskForSchema(t), { merge: true }));
    await batch.commit();
    markSaved();
    console.info(`Schema migration applied to ${candidates.length} task(s).`);
  } catch (e) {
    markSaveError();
