...
16. **모바일 목록 서브태스크 접기 동작 수정**
   * 지연 서브태스크가 있으면 자동 펼침 조건 때문에 `접기`를 눌러도 다시 펼쳐지던 문제를 확인하고, 사용자 명시 접힘 상태를 `collapsedTaskIds`로 분리해 접기 동작이 우선되도록 수정 완료.
   * 서브태스크 토글 버튼에 실제 렌더 상태를 나타내는 `data-expanded`를 추가하고, 클릭 핸들러가 이 값을 기준으로 `expandedTaskIds`/`collapsedTaskIds`를 갱신하도록 보완 완료.
   * `index.html`의 `state.js`, `table-mobile-renderer.js`, `app.js` 캐시 버전을 각각 `v20260711-v4`, `v20260711-v11`, `v20260711-v11`로 갱신.
17. **모바일 캘린더 연간 보기 실제 브라우저 QA 완료**
   * 실제 모바일 브라우저 또는 DevTools 모바일 폭에서 `년간` 탭을 열어 막대 간 겹침, 터치 목표 크기, 긴 업무명 truncation 상태를 육안 확인 완료.
   * 실제 데이터에서 본 업무 중심 전환 문구(`총 n개 · 본 업무`, `주요 n/전체`)가 자연스럽게 표시되는지 확인 완료.
18. **모바일 핵심 화면 회귀 스모크 체크 정리**
   * `docs/mobile_qa_checklist.md`에 모바일 `목록`/`캘린더 월간`/`캘린더 년간`/`월별 요약` 최소 QA 체크리스트를 작성 완료.
   * `scripts/mobile-smoke.js`를 추가해 `jsdom` 기반으로 모바일 목록 카드, 월간 카드, 연간 간트 밀집 배지, 월별 요약 메모 섹션을 반복 검증할 수 있도록 자동화 완료.
   * `package.json`에 `npm run smoke:mobile` 스크립트를 추가해 다음 세션에서도 같은 검증을 재실행할 수 있도록 정리 완료.
   * `npm run smoke:mobile`, `node --check scripts/mobile-smoke.js`, `git diff --check` 검증 통과 완료.
   * 로컬 정적 서버와 Headless Chrome 390x844 모바일 폭 스크린샷으로 앱 첫 화면 로드 및 주요 버튼 배치가 깨지지 않는 것을 확인 완료.

---

## 🚀 다음 진행할 작업 (Next Steps)
### 1. 모바일 캘린더 연간 보기 실제 브라우저 육안 확인 (QA)
- [x] 실제 모바일 브라우저 또는 DevTools 모바일 폭에서 `년간` 탭을 열어 막대 간 겹침, 터치 목표 크기, 긴 업무명 truncation 상태 최종 확인
- [x] 실제 데이터에서 본 업무 중심 전환 문구(`총 n개 · 본 업무`, `주요 n/전체`)가 자연스러운지 확인

### 2. 모바일 핵심 화면 회귀 스모크 체크 정리 (추천)
- [x] 모바일 `목록`/`캘린더 월간`/`캘린더 년간`/`월별 요약` 화면을 한 번에 확인하는 최소 QA 체크리스트 작성
- [x] 가능한 범위에서 `jsdom` 기반 렌더 스모크 또는 간단한 브라우저 스모크 스크립트로 반복 검수 포인트 자동화
- [x] 체크리스트 또는 스크립트 실행 결과를 기준으로 다음 UI 보정 필요 여부 판단

### 3. 모바일 스모크 결과 기반 UI 보정 여부 판단
- [x] `npm run smoke:mobile` 실행 결과를 확인하고 실패 시 해당 렌더러 보정
- [x] Headless Chrome 모바일 폭 첫 화면 확인 및 `jsdom` 핵심 렌더 스모크 기준 추가 UI 보정 불필요 판단

### 4. 캘린더 및 간트 차트 실시간 통합 필터링 구현
...
### 5. 태스크 상세 변경 이력(Activity Log) 추적 및 타임라인 뷰 추가
...
### 6. 트래커별 미니멀 KPI 목표 배지 구현 (Option A)
...
### 7. 트래커별 커스텀 KPI 매니저 및 전용 입력 모달창 구현
...
### 8. 코드 품질 및 기술 부채 개선 백로그 (에이전트 권장)
